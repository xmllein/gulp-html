...
# 部署上线gitlab地址
feReleaseGit = 'https://github.com/xmllein/gulp-html-release.git'
...
def releaseOnline():
    ...
    os.chdir(currPath)
    exeCmd('rm -rf ' + os.path.join(feRelease + 'gulp-html-release', "*"))

    # 拷贝到gitlab目录
    cmd = 'scp -r ' + os.path.join(bakTmp, '', '*') + ' ' + \
        os.path.join(feRelease + 'gulp-html-release', '')
    exeCmd(cmd)

    # 切到fe-release git push
    os.chdir(os.path.join(currPath, feRelease + '/gulp-html-release', ''))
    exeCmd('git add .')
    ...
    print 'release to gulp-html-release end'
